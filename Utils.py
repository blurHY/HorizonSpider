...
def filter_link(link):
    ...
    validUrl = re.match(r"^[A-Za-z0-9-._~:/?#\[\]@!$&'()*+,;=%\\]+$", link)
    return prefix and validUrl
...
def shorted_url(url):
    try:
        return re.sub("^http:\/\/127\.0\.0\.1:[0-9]+\/", "", url)
    except:
        return url
...
def filter_zerosites(siteaddr):
    return re.match("^[A-Za-z0-9]{33,36}$", siteaddr)


def filter_link_crawled(url):
    reg = re.match("^\s*https?://127\.0\.0\.1:43110/", url)
    return reg