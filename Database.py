...
class Database:
    ...
    def insert_main_item(self, **kwargs):
        cursor = self.db.cursor()
        self.db.ping()
        keys = ",".join(key for key in kwargs.keys())
        values = ",".join("'" + escape_string(str(value)) +
        ...
        eqs = ",".join("{0}=values({0})".format(key)
                       for key in kwargs.keys())
        query = "insert into main({0}) values ({1})".format(
            keys, values, eqs)
        cursor.execute(query)
        cursor.close()
    ...
    def crawl_duration_add(self, duration):
        try:
            cursor = self.db.cursor()
            self.db.ping()
            cursor.execute("insert into crawl_duration values(%s)", [duration])
            cursor.close()
            self.commit_or_rollback()
        except IntegrityError as ie:
            self._error_info(ie)
